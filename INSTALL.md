...
If `~/.config/hypr/hyprland.conf` is a symlink, it is probably generated by
NixOS or Home Manager. The installer never modifies symlinked configurations
or files under `/nix/store`.

During interactive setup, choose the recommended separate-file option. The
installer writes the managed bindings to:

```text
~/.config/hypr/listen.conf
```

Add this line to the declarative Hyprland configuration managed by Nix or Home
Manager, then rebuild and reload Hyprland:

```ini
source = /home/YOUR_USER/.config/hypr/listen.conf
```

For automation, the same behavior is available with:

```bash
./install.sh --integration hyprland --hyprland-bindings yes \
  --hyprland-config-mode separate
```

You can choose `--hyprland-config-mode skip` and define the bindings entirely
in Nix/Home Manager, or use `--hyprland-config /path/to/writable.conf` for a
different writable configuration file. Uninstall removes the managed block
from a writable file, but does not delete a user configuration file.
