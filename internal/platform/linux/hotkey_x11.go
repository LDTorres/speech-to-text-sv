...
func (s *HotkeySource) Start(ctx context.Context) error {
	...
	if err != nil {
		cancel()
		return fmt.Errorf("create linux hotkey binding: %w", err)
	}

	if err := binding.Register(); err != nil {
		cancel()
		return fmt.Errorf("register linux hotkey: %w", err)
	}
...
func (s *HotkeySource) Stop(ctx context.Context) error {
	...
	case err := <-unregisterDone:
		if err != nil {
			return fmt.Errorf("unregister linux hotkey: %w", err)
		}
		return nil
	case <-ctx.Done():
		return ctx.Err()
...
func parseHotkeyModifiers(modifiers []string) ([]hotkey.Modifier, error) {
	...
	for _, modifier := range modifiers {
		...
		case "alt":
			parsed = append(parsed, hotkey.Mod1)
		default:
			return nil, fmt.Errorf("unsupported linux hotkey modifier %q", modifier)
		}
	}
...
func parseHotkeyKey(key string) (hotkey.Key, error) {
	...
	default:
		return 0, fmt.Errorf("unsupported linux hotkey key %q", key)
	}
}
